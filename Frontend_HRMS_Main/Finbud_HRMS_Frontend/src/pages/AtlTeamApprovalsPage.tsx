...
/**
 * ATL "Team Daily Approvals" console.
 *
 * Shows the ATL's own direct reports' SUBMITTED daily commitments with
 * Approve / Reject actions, plus a team snapshot for any date. Every manager
 * endpoint is scoped to the ATL's own team by the backend, so an ATL can only
 * ever act on the employees assigned under them.
 */
...
export const AtlTeamApprovalsPage = () => {
  ...
  const { data: teamForDate = [] } = useQuery<DailyCommitment[]>({
    queryKey: ['atl-team-for-date', managerId, date],
  ...
  const reviewMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['atl-pending-commitments', managerId] });
      queryClient.invalidateQueries({ queryKey: ['atl-team-for-date', managerId] });
    },
  });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Team Daily Approvals"
        description="Approve or reject your team's submitted daily commitments, and see a snapshot for any date."
      />
