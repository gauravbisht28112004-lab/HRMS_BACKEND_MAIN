...
/**
 * ATL "Assign Monthly Targets" page.
 *
 * Lets an Assistant Team Leader set / update a monthly disbursal target for
 * each employee assigned under them (Employee.manager == this ATL). The
 * backend restricts an ATL to their own direct reports; ADMIN/HR/MANAGER stay
 * unrestricted. Employees still enter their own daily commitments — the
 * "achieved" overlay here is computed server-side from APPROVED daily rows.
 *
 * Mirrors the Team Leader monthly page: a live team table plus an inline form
 * that prefills from the selected employee's existing target.
 */
const NOW = new Date();
...
export const AtlMonthlyTargetPage = () => {
  ...
  const { data: teamMembers = [] } = useQuery<Employee[]>({
    queryKey: ['atl-team', managerId],
    queryFn: () => (managerId ? api.employees.listByManager(managerId) : Promise.resolve([])),
    enabled: Boolean(managerId),
  });

  // -- Monthly targets for the picked period (backend scopes to my team) --
  const { data: targets = [], isLoading } = useQuery<MonthlyTarget[]>({
    queryKey: ['atl-monthly-targets', managerId, yearNum, monthNum],
    queryFn: () =>
      managerId
  ...
  const upsertMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setErrorMsg(null);
      queryClient.invalidateQueries({ queryKey: ['atl-monthly-targets', managerId, yearNum, monthNum] });
      queryClient.invalidateQueries({ queryKey: ['atl-team-monthly', managerId] });
    },
    onError: (err: unknown) => {
  ...
  if (!managerId) {
    return (
      <Card className="border border-amber-200 bg-amber-50">
        <p className="text-sm font-semibold text-amber-800">ATL profile not linked</p>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Assign Monthly Targets"
        description="Set a monthly disbursal target for each member of your team. Employees fill in their own daily commitments; you assign the monthly goal."
      />
