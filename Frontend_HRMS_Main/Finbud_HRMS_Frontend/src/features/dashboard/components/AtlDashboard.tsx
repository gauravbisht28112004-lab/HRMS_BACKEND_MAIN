...
/**
 * ATL (Assistant Team Leader) dashboard — real-time view of the ATL's own
 * direct reports: pending approvals, monthly-target progress, and the
 * org-wide goal. Mirrors the Team Leader dashboard; every API call is scoped
 * to the ATL's own team by the backend.
 */
const NOW = new Date();
...
export const AtlDashboard = () => {
  ...
  const { data: pending = [] } = useQuery<DailyCommitment[]>({
    queryKey: ['atl-pending-commitments', managerId],
  ...
  const { data: todayCommitments = [] } = useQuery<DailyCommitment[]>({
    queryKey: ['atl-team-today', managerId, TODAY],
  ...
  const { data: monthlyProgress = [] } = useQuery<MonthlyTarget[]>({
    queryKey: ['atl-team-monthly', managerId, currentYear, currentMonth],
    queryFn: () =>
      managerId
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <Card className="border border-slate-200 bg-white shadow-none">
          <div className="flex items-center justify-between">
            ...
            <Link to="/atl-daily-commitment" className="text-xs font-medium text-brand-700 hover:underline">
              Manage approvals →
            </Link>
          </div>
          <div className="mt-5">
        ...
        <Card className="border border-slate-200 bg-white shadow-none">
          <div className="flex items-center justify-between">
            ...
            <Link to="/atl-monthly-target" className="text-xs font-medium text-brand-700 hover:underline">
              Manage targets →
            </Link>
          </div>
          {monthlyProgress.length === 0 ? (
            <p className="mt-4 text-sm text-slate-500">
              No monthly targets set for your team yet. Set them from the My Team Commitment page.
            </p>
          ) : (
